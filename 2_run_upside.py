...
current_dir = config["upside_simulation"]["output_path"]
for protein in config["upside_simulation"]["proteins"]:
    files = os.listdir(f"{current_dir}/{protein}/top_N_fibrils")
    name = f"TopDesigns_{protein}"
    for file in files:
        ...
        pdb_id           = file
        pdb_dir          = f"{current_dir}/{protein}/top_N_fibrils/{file}"
        print(pdb_dir)
        sim_id           = 'Simulation1'
        is_native        = True
        ff               = 'ff_2.1'

        duration         = config["upside_simulation"]["n_steps"]
        frame_interval   = 10
        work_dir         = f"{current_dir}/{protein}/Simulations"
        ...
        n_rep            = config["upside_simulation"]["n_reps"]     # replica number
        T_low            = 0.70 
        T_high           = 1.20
        replica_interval = 100    # How long takes an exchange attempt (upside time unit)
        ...
        account          = config["upside_simulation"]["userid"]    # FIXME change it 
        partition        = config["upside_simulation"]["partition"]  # FIXME change it
        job_name         = '{}_{}'.format(pdb_id, sim_id)
        run_time         = "10:00:00" # requested run time of job allocation in hh:mm:ss
        ...
        #link the top N fibrils to the input directory
        output_dir = "{}/outputs".format(work_dir)
        run_dir    = "{}/{}".format(output_dir, sim_id) 

        make_dirs = [input_dir, output_dir, run_dir, work_dir]
        for direc in make_dirs:
            if not os.path.exists(direc):
                os.makedirs(direc)
        os.symlink(pdb_dir, os.path.join(input_dir, pdb_id))
        ...
        log_folder = f"{current_dir}/{protein}/logs"
        # SLURM options
        ...
        sbatch_opts = (
                        ...
                        "--nodes=1 "
                        "--ntasks-per-node={} "
                        "--output={log_folder}/upside_%j.out"
                        "--error={log_folder}/upside_%j.err"
                    )
